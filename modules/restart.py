...
from aqt.qt import QAction, QProcess, QTimer
...
def _build_target_cmd() -> list[str]:
    ...
    for base in (app_path, sys_exe, argv0):
        ...
        if found:
            logging.dbg("restart target: using anki.exe", found, source="restart")
            return [found]

    for base in (app_path, sys_exe):
        b = str(base or "").strip().lower()
        if b.endswith("\\ankiw.exe") or b.endswith("/ankiw.exe"):
            logging.dbg("restart target: using direct ankiw path", base, source="restart")
            return [str(base)]
        if b.endswith("\\anki.exe") or b.endswith("/anki.exe"):
            ...
            if gui_candidate and os.path.exists(gui_candidate):
                logging.dbg("restart target: switched direct anki->ankiw", gui_candidate, source="restart")
                return [gui_candidate]
            logging.dbg("restart target: using direct anki path", base, source="restart")
            return [str(base)]

    if app_path:
        low = app_path.lower()
        if (low.endswith("\\pythonw.exe") or low.endswith("/pythonw.exe")) and not args:
            logging.dbg("restart target: fallback pythonw -m aqt", app_path, source="restart")
            return [app_path, "-m", "aqt"]
        logging.dbg("restart target: fallback app path", app_path, source="restart")
        return [app_path] + args
    if sys_exe:
        logging.dbg("restart target: fallback sys.executable", sys_exe, source="restart")
...
def _start_restart_helper(target_cmd: list[str], delay_seconds: int) -> bool:
    ...
    if not primary_cmd and not fallback_cmd:
        logging.warn("restart helper: missing helper files", source="restart")
        return False
    ...
    def _spawn_and_probe(cmd: list[str], label: str) -> bool:
        try:
            ...
            rc = proc.poll()
            if rc is not None:
                logging.warn("restart helper: process exited immediately", label, f"rc={rc}", source="restart")
                return False
            logging.dbg("restart helper: started via subprocess", label, cmd[0], source="restart")
            return True
        except Exception as exc:
            logging.warn("restart helper: subprocess launch failed", label, repr(exc), source="restart")
            return False
    ...
    for cmd, label in ((primary_cmd, "primary"), (fallback_cmd, "python-fallback")):
        ...
        try:
            if QProcess.startDetached(cmd[0], cmd[1:]):
                logging.dbg("restart helper: started via QProcess", label, cmd[0], source="restart")
                return True
        except Exception as exc:
            logging.warn("restart helper: QProcess failed", label, repr(exc), source="restart")

    logging.error("restart helper: all start methods failed", source="restart")
    return False
...
def _delayed_restart_anki() -> None:
    if mw is None:
        return
    try:
        logging.dbg("restart click: begin", source="restart")
        target_cmd = _build_target_cmd()
        if not target_cmd:
            logging.warn("restart click: no target cmd", source="restart")
        ...
        try:
            ...
            logging.warn("restart click: mw.close raised", repr(exc), source="restart")
        logging.dbg("restart click: close requested, ret=", repr(close_ret), source="restart")
        ...
        def _force_quit() -> None:
            ...
            try:
                ...
                if app is not None:
                    app.quit()
                    logging.dbg("restart click: app.quit requested", source="restart")
            except Exception as exc:
                logging.warn("restart click: app.quit failed", repr(exc), source="restart")

        QTimer.singleShot(300, _force_quit)
    except Exception as exc:
        logging.error("restart click: exception", repr(exc), source="restart")
